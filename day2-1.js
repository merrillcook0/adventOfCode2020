var database = [['5-10','b:','bhbjlkbbbbbbb'],
['3-4','j:','hjvj'],
['8-9','p:','pmljtsttp'],
...
var hash_database = [];
function lowest(db){
	var lowest = db.split("-")[0];
  return lowest;

}
function highest(db){
	var highest = db.split("-")[2];
  return highest;

}
function letter(db){
var letter = db.split(":")[0];
return letter;
}
function is_char(char, string){
	var count = 0;
...
var valid_pw_counter = 0;
for (var a=0;a<database.length;a++){
	//console.log(lowest(database[a][0]));
  is_valid(lowest(database[a][0],highest(database[a][0], letter(database[a][1],database[2]))));
